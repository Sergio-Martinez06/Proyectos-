...
private:
    ...
    void buscarNumeroRecursivo(Nodo* nodo, int objetivo, string camino, vector<string>& rutas, int nivel) {
        ...
        if (nodo->valor == objetivo) {
            rutas.push_back(camino + to_string(nodo->valor) + " (Nivel " + to_string(nivel) + ")");
        }

        if (nodo->izquierdo != nullptr) {
            string nuevaRuta = camino + to_string(nodo->valor) + " -> (izquierda) ";
            buscarNumeroRecursivo(nodo->izquierdo, objetivo, nuevaRuta, rutas, nivel + 1);
        }

        if (nodo->derecho != nullptr) {
            string nuevaRuta = camino + to_string(nodo->valor) + " -> (derecha) ";
            buscarNumeroRecursivo(nodo->derecho, objetivo, nuevaRuta, rutas, nivel + 1);
        }
    }
...
public:
    ...
    void buscarNumero() {
        ...
        int objetivo = leerEntero("Ingrese el número que desea buscar: ");
        vector<string> rutas;
